...
const SectionGridMoreExplore: FC<SectionGridMoreExploreProps> = ({
  ...
  const [tabActive, setTabActive] = useState(groupCollections?.[0]?.id || 'fallback-1')
  ...
  return (
    <div className={clsx('relative', className)}>
      ...
      {groupCollections && groupCollections.length > 0 && (
      ...
      <div className={`grid gap-4 md:gap-7 ${gridClassName}`}>
        {groupSelected?.collections?.map((collection, index) => (
          ...
          <div className="col-span-full text-center py-12">
            <p className="text-neutral-500">No collections available at the moment.</p>
          </div>
        )}
      </div>
